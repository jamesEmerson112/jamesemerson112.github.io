...
import { formatNumber, formatCurrency, formatYears, formatDuration } from './formatters.js';
import {
  getLanguageColor,
...
describe('language utility exports', () => {
  it('exports all language utility functions', () => {
    expect(typeof getLanguageColor).toBe('function');
    expect(typeof getContrastTextColor).toBe('function');
    expect(typeof getDisplayPrimaryLanguage).toBe('function');
    expect(typeof resolveDominantProgrammingLanguage).toBe('function');
    expect(typeof buildProgrammingComposition).toBe('function');
    expect(typeof isNonProgrammingLanguage).toBe('function');
  });

  it('returns dark text for bright language colors', () => {
    expect(getContrastTextColor('#f7df1e')).toBe('#0f172a');
  });

  it('returns light text for dark language colors', () => {
    expect(getContrastTextColor('#1e293b')).toBe('#f8fafc');
  });

  it('falls back safely for invalid colors', () => {
    expect(getContrastTextColor('invalid-color')).toBe('#f8fafc');
  });

  it('resolves dominant programming language by scanning language list when primary is non-programming', () => {
    expect(
      resolveDominantProgrammingLanguage('JSON', [
        { name: 'JSON', code: 400 },
  ...
  it('falls back to lines and deterministic name tie-break when code values are equal', () => {
    expect(
      resolveDominantProgrammingLanguage('Plain Text', [
  ...
  it('builds programming composition with non-programming and tail bucketed into Other', () => {
    const rows = buildProgrammingComposition([
  ...
  it('maps display primary language using dominant programming resolver', () => {
    expect(getDisplayPrimaryLanguage('TypeScript')).toBe('TypeScript');
    expect(
      getDisplayPrimaryLanguage('Plain Text', [
        { name: 'JSON', code: 400 },
        { name: 'Python', code: 300 }
      ])
    ).toBe('Python');
    expect(getDisplayPrimaryLanguage('Plain Text')).toBe('N/A');
  });
});

describe('buildProgrammingComposition edge cases', () => {
  it('returns empty array for empty input', () => {
    expect(buildProgrammingComposition([])).toEqual([]);
  });
...
describe('resolveDominantProgrammingLanguage edge cases', () => {
  it('returns N/A for empty string primary and empty languages', () => {
    expect(resolveDominantProgrammingLanguage('', [])).toBe('N/A');
  });

  it('returns N/A for null primary', () => {
    expect(resolveDominantProgrammingLanguage(null)).toBe('N/A');
  });

  it('returns N/A for undefined primary', () => {
    expect(resolveDominantProgrammingLanguage(undefined)).toBe('N/A');
  });

  it('returns valid primary without needing language list', () => {
    expect(resolveDominantProgrammingLanguage('Rust')).toBe('Rust');
  });
});
...
describe('formatDuration edge cases', () => {
  ...
  it('returns 0 mo for negative values', () => {
    expect(formatDuration(-5)).toBe('0 mo');
  });

  it('returns 0 mo for NaN', () => {
    expect(formatDuration(NaN)).toBe('0 mo');
  });
  ...
  it('formats months with one decimal', () => {
    expect(formatDuration(6)).toBe('6.0 mo');
  });

  it('formats 24 months as years', () => {
    expect(formatDuration(24)).toBe('2.0 yrs');
  });

  it('rounds large year values', () => {
    expect(formatDuration(120)).toBe('10 yrs');
  });
});
